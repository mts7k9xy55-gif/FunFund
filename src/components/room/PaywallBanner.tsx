          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                alert(language === "ja" ? "無料試用モードで有効化しました" : "Activated in free trial mode");
              } catch (error: any) {
                alert(error.message || (language === "ja" ? "有効化に失敗しました" : "Activation failed"));
              } finally {
                setIsActivating(false);
              }
            }}
            disabled={isActivating}
            className="ml-auto px-3 py-1.5 bg-emerald-600 text-white rounded text-xs font-medium hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isActivating
              ? (language === "ja" ? "有効化中..." : "Activating...")
              : (language === "ja" ? "無料で試す" : "Try for Free")}
          </button>
        ) : (
          <button
            onClick={async () => {
